...
const BollywoodLoveSongs = () => {
  ...
  const fetchLoveSongs = async () => {
    try {
      ...
      // Attempt to fetch from JioSaavn with Bollywood love songs query
      const saavnRes = await fetch(`/api/saavn/search?query=Bollywood+Love+Songs`);
      if (saavnRes.ok) {
      ...
      // Fallback to SoundCloud with Bollywood love songs query
      const scRes = await fetch(`/api/soundcloud?q=Bollywood+Love+Songs&limit=8`);
      if (scRes.ok) {
      ...
      throw new Error("No Bollywood love songs found from JioSaavn or SoundCloud.");
    } catch (err) {
      console.error("Error fetching Bollywood love songs:", err);
      setError("Failed to fetch Bollywood love songs.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="container mx-auto px-4 py-10">
      <h2 className="text-3xl font-bold mb-6">❤️ Bollywood Love Songs</h2>
...
export default BollywoodLoveSongs;