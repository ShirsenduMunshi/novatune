...
const TrendingMusics = () => {
  ...
  const fetchTrendingTracks = async () => {
    try {
      ...
      // Attempt to fetch from JioSaavn
      const saavnRes = await fetch(`/api/saavn/search?query=Trending`);
      if (saavnRes.ok) {
      ...
      const scRes = await fetch(`/api/soundcloud?q=top music&limit=8`);
      if (scRes.ok) {
      ...
      throw new Error("No tracks found from JioSaavn or SoundCloud.");
    } catch (err) {
      console.error("Error fetching tracks:", err);
      setError("Failed to fetch trending music.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="container mx-auto px-4 py-10">
      <h2 className="text-3xl font-bold mb-6">🔥 Trending Musics</h2>
