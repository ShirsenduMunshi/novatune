...
const NinetiesHits = () => {
  ...
  const fetchNinetiesHits = async () => {
    try {
      setLoading(true);
      setError(null);
      setTracks([]);

      // Attempt to fetch from JioSaavn with 90s hits query
      const saavnRes = await fetch(`/api/saavn/search?query=90s+Hits`);
      if (saavnRes.ok) {
      ...
      // Fallback to SoundCloud with 90s hits query
      const scRes = await fetch(`/api/soundcloud?q=90s+Hits&limit=8`);
      if (scRes.ok) {
      ...
      throw new Error("No 90s hits found from JioSaavn or SoundCloud.");
    } catch (err) {
      console.error("Error fetching 90s hits:", err);
      setError("Failed to fetch 90s hits.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="container mx-auto px-4 py-10">
      <h2 className="text-3xl font-bold mb-6">🌟 90s Hits</h2>
